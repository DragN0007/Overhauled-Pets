...
@OnlyIn(Dist.CLIENT)
public class RingneckOnShoulderLayer<T extends Player>
        extends RenderLayer<T, PlayerModel<T>> {

    private final RingneckRender geckoRenderer;
    private Ringneck leftDummy;
    private Ringneck rightDummy;

    public RingneckOnShoulderLayer(RenderLayerParent<T, PlayerModel<T>> parent, EntityRendererProvider.Context ctx) {
        super(parent);
        this.geckoRenderer = new RingneckRender(ctx);
    }
    ...
    private void renderOneShoulder(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T player, float partialTicks, boolean left) {
        ...
        Ringneck dummy = left ? leftDummy : rightDummy;
        if (dummy == null) {
            dummy = new Ringneck(POEntityTypes.RINGNECK_ENTITY.get(), player.level());
            if (left) leftDummy = dummy; else rightDummy = dummy;
        }
        if (EntityType.byString(tag.getString("id")).filter(t -> t == POEntityTypes.RINGNECK_ENTITY.get()).isEmpty()) {
            return;
        }
