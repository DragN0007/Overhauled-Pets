...
public class PORecipeMaker extends RecipeProvider implements IConditionBuilder {
    ...
    public void buildRecipes(Consumer<FinishedRecipe> pFinishedRecipeConsumer) {
        ...
        for (DyeColor color : DyeColor.values()) {
            ShapedRecipeBuilder.shaped(RecipeCategory.MISC, POItems.VESTS.get(color).get())
                    .define('A', DyeItem.byColor(color))
                    .define('B', ItemTags.WOOL)
                    .pattern("ABB")
                    .unlockedBy("has_wool", inventoryTrigger(ItemPredicate.Builder.item()
                    .of(ItemTags.WOOL).build()))
                    .save(pFinishedRecipeConsumer, new ResourceLocation(LivestockOverhaul.MODID, color.getName() + "_vest"));
        }
